...
def generate_test_report(categorized, all_questions, output_path):
    ...
    with open(output_path, "w") as f:
        ...
        f.write(
            f"- High confidence matches (≥0.85): **{len(categorized['auto_accept'])}** pairs\n"
        )
        f.write(
            f"- Manual review needed (0.75-0.85): **{len(categorized['manual_review'])}** pairs\n"
        )
        f.write(
            f"- Low confidence (<0.75): **{len(categorized['low_confidence'])}** pairs\n\n"
        )
        ...
        for i, match in enumerate(
            ...
            f.write(
                f"**{match['wave1']}.{match['var1']}**: {match['question1'][:100]}...\n\n"
            )
            f.write(
                f"**{match['wave2']}.{match['var2']}**: {match['question2'][:100]}...\n\n"
            )
