...
class QuestionMatcher:
        ...
        for idx, question in enumerate(self.questions):
            ...
            for sim, match_idx in zip(similarities[0][1:], indices[0][1:]):
                ...
                if match['wave'] != question['wave']:
                    ...
                    if self._check_target_mismatch(question['question_text'], match['question_text']):
    ...
    def _check_target_mismatch(self, text1, text2):
        ...
        target_keywords = {
            'relatives': ['relative', 'relatives', 'family member', 'family members', 'kinship'],
            ...
            'colleagues': ['colleague', 'colleagues', 'coworker', 'coworkers', 'workmate'],
            'government': ['government official', 'officials', 'bureaucrat', 'civil servant'],
            ...
            'parliament': ['parliament', 'legislature', 'congress', 'national assembly'],
...
def generate_detailed_report(categorized, output_path):
    ...
    with open(output_path, 'w') as f:
        ...
        f.write(f"**Count**: {len(categorized['auto_accept'])} unique question pairs\n\n")
        ...
        for i, match in enumerate(sorted(categorized['auto_accept'],
                                        key=lambda x: x['similarity'], reverse=True), 1):
        ...
        f.write(f"**Count**: {len(categorized['manual_review'])} unique question pairs\n\n")
    ...
    print(f"✓ Detailed report saved: {len(categorized['auto_accept']) + len(categorized['manual_review'])} total pairs")
...
def generate_summary_statistics(categorized, all_questions, output_path):
    ...
    with open(output_path, 'w') as f:
        ...
        f.write(f"- **Total unique pairs identified**: {len(categorized['auto_accept']) + len(categorized['manual_review']):,}\n\n")

        f.write("## Match Quality Distribution\n\n")
        f.write(f"| Category | Pair Count | % of Matched Pairs |\n")
        f.write(f"|----------|------------|--------------------|\n")

        total_pairs = len(categorized['auto_accept']) + len(categorized['manual_review'])
        auto_pct = 100 * len(categorized['auto_accept']) / total_pairs if total_pairs > 0 else 0
        manual_pct = 100 * len(categorized['manual_review']) / total_pairs if total_pairs > 0 else 0

        f.write(f"| **High confidence (≥0.85)** | **{len(categorized['auto_accept']):,}** | **{auto_pct:.1f}%** |\n")
        f.write(f"| **Manual review (0.75-0.85)** | **{len(categorized['manual_review']):,}** | **{manual_pct:.1f}%** |\n")
        f.write(f"| Low confidence (<0.75) | {len(categorized['low_confidence']):,} | Not reported |\n\n")

        f.write("## Question Coverage\n\n")
        f.write(f"| Category | Question Count | % of Total |\n")
        f.write(f"|----------|----------------|------------|\n")
        ...
        f.write(f"| Questions in auto-matches | {len(auto_vars):,} | {auto_q_pct:.1f}% |\n")
        f.write(f"| Questions in manual review | {len(manual_vars):,} | {manual_q_pct:.1f}% |\n")
        f.write(f"| **Total matched questions** | **{len(matched_vars):,}** | **{matched_pct:.1f}%** |\n")
        f.write(f"| Wave-specific (no match) | {len(unmatched_vars):,} | {unmatched_pct:.1f}% |\n\n")
        ...
        f.write(f"| Wave | Total Questions |\n")
        f.write(f"|------|----------------:|\n")
        ...
        f.write(f"| Wave Pair | Auto-Accept | Manual Review | Total |\n")
        f.write(f"|-----------|------------:|--------------:|------:|\n")
        ...
        f.write("\n## Key Insights\n\n")
        f.write(f"1. **Coverage**: {matched_pct:.1f}% of questions have cross-wave matches\n")
        f.write(f"2. **Automation**: {auto_pct:.1f}% of matches are high-confidence (auto-accept)\n")
        f.write(f"3. **Manual effort**: {len(categorized['manual_review']):,} pairs need human review\n")
        f.write(f"4. **Wave-specific**: {len(unmatched_vars):,} questions are unique to their wave\n\n")
