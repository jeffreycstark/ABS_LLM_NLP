...
def fix_missing_codes(content, wave_name):
    ...
    pattern = r'(mutate\((\w+)_reversed\s*=\s*if_else\([^)]+\s+(safe_reverse_\w+)\((\w+)\))'
    ...
    content = re.sub(
        r'missing_codes\s*=\s*c\([^)]+\)',
        f'missing_codes = {LIKERT_MISSING}',
        content
    )
...
def main():
    ...
    for filename, wave_name in wave_files:
        ...
        if wave_name in TENPOINT_VARS:
            print(f"✓ Applied 10-point scale handling for: {', '.join(TENPOINT_VARS[wave_name])}")

        # Count changes
        likert_count = content.count(LIKERT_MISSING)
        tenpoint_count = content.count(TENPOINT_MISSING) if wave_name in TENPOINT_VARS else 0
