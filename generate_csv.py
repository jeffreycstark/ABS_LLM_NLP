...
def json_to_csv(enriched_json_file: str, output_csv: str, output_detailed_csv: str):
    ...
    with open(output_detailed_csv, 'w', encoding='utf-8', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['variable_id', 'domain', 'concepts', 'question_text', 'value_labels'])
        ...
        for var in variables:
            ...
            labels_str = '; '.join([f"{vl['value']}={vl['label']}" for vl in value_labels])
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python generate_csv.py <enriched_json> <output_csv> <output_detailed_csv>")
        print("Example: python generate_csv.py W5_enriched.json W5_concepts.csv W5_concepts_detailed.csv")
        sys.exit(1)
