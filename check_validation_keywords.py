...
def extract_content_words(text: str, min_length: int = 5) -> Set[str]:
    ...
    stopwords = {
        'about', 'after', 'again', 'against', 'being', 'before', 'below',
        'between', 'doing', 'during', 'further', 'having', 'should', 'their',
        'there', 'these', 'those', 'through', 'under', 'which', 'while',
        'would', 'could', 'following', 'statement', 'question', 'agree',
        'disagree', 'strongly', 'somewhat', 'please', 'think', 'opinion'
    }
...
def find_unique_keywords(questions: List[Dict], wave: str) -> Dict[str, Dict]:
    ...
    for var_id, words in question_to_words.items():
        ...
        q_text = next(q['question_text'] for q in questions if q.get('variable_id') == var_id)
        ...
        results[var_id] = {
            ...
            'has_validation_keyword': len(unique_words) > 0 or len(distinctive_words) > 0
        }
...
def analyze_wave(wave_name: str) -> Dict:
    ...
    try:
        ...
        return {
            ...
            'percentage_covered': (with_keywords / total_questions * 100) if total_questions > 0 else 0,
            'questions': results
        }
...
def main():
    ...
    for wave in waves:
        ...
        print(f"  With validation keywords: {result['with_keywords']} ({result['percentage_covered']:.1f}%)")
    ...
    print(f"  With validation keywords: {total_with} ({total_with/total_q*100:.1f}%)")
