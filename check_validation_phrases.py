...
def find_distinctive_phrase(
    question_text: str, all_questions: List[str], min_words: int = 2, max_words: int = 4
) -> Tuple[str, int]:
    """
...
def main():
    ...
    for wave in waves:
        ...
        print(
            f"  Unique phrases (appears in 1 question only): {result['unique_phrases']}"
        )
        print(
            f"  Acceptable phrases (appears in ≤2 questions): {result['acceptable_phrases']}"
        )
        print(f"  Problematic (appears in >2 questions): {result['problematic']}")

        coverage = (
            (result["acceptable_phrases"] / result["total_questions"] * 100)
        ...
        for var_id, info in sorted(result["questions"].items()):
            if info["is_unique"] and info["validation_phrase"]:
                q_text_short = (
                    info["question_text"][:60] + "..."
                    if len(info["question_text"]) > 60
                    else info["question_text"]
                )
    ...
    print(
        f"  Acceptable phrases: {total_acceptable} ({total_acceptable / total_q * 100:.1f}%)"
    )
    print(
        f"  Problematic: {total_problematic} ({total_problematic / total_q * 100:.1f}%)"
    )
