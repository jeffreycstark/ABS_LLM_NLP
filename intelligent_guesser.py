...
class IntelligentGuesser:
    ...
    NA_PATTERNS = [
        r'\bna\b', r'\bn/?a\b', r'missing', r'no answer', r'decline', r'refused',
        r'don\'t know', r'can\'t choose', r'not applicable', r'inapplicable',
        r'don\'t understand', r'do not understand', r'don not understand'
    ]

    # Positive sentiment indicators
    POSITIVE_WORDS = [
        'agree', 'strongly agree', 'satisfied', 'very satisfied', 'excellent',
        'very good', 'good', 'always', 'often', 'frequently', 'support',
        'approve', 'favorable', 'positive', 'confident', 'trust', 'easy',
        'very easy', 'important', 'very important', 'yes', 'definitely'
    ]

    # Negative sentiment indicators
    NEGATIVE_WORDS = [
        'disagree', 'strongly disagree', 'dissatisfied', 'very dissatisfied',
        'poor', 'very poor', 'bad', 'never', 'rarely', 'seldom', 'oppose',
        'disapprove', 'unfavorable', 'negative', 'distrust', 'difficult',
        'very difficult', 'unimportant', 'not important', 'no'
    ]
    ...
    def classify_scale(self, value_labels: List[Dict]) -> ScaleAnalysis:
        ...
        if has_value_7:
            ...
            if values_before_7:
                ...
                substantive_values = [vl for vl in value_labels
                                     if 0 < vl['value'] <= max_substantive]
                # NA values: 7 and above, plus negative
                na_values = [vl for vl in value_labels
                            if vl['value'] < 0 or vl['value'] >= 7]
            else:
        ...
        value_1_label = next((vl['label'] for vl in substantive_values if vl['value'] == min_value), '')
        max_value_label = next((vl['label'] for vl in substantive_values if vl['value'] == max_value), '')
        ...
        if max_value <= 2:
            scale_type = 'binary'
            reasoning = f'Max substantive value is {max_value} (binary yes/no or 1-2 scale)'
    ...
    def analyze_questionnaire(self, atomic_json_file: str, output_file: str):
        ...
        print(f"\n{'='*60}")
        print(f"SCALES NEEDING REVERSAL: {stats['needs_reversal']} ({(stats['needs_reversal']/len(variables))*100:.1f}%)")
        print(f"{'='*60}")
        ...
        for scale_type in ['binary', 'likert_5', 'scale_10', 'categorical']:
            examples = [v for v in analyzed if v['scale_analysis']['scale_type'] == scale_type]
            if examples:
                ex = examples[0]
...
def main():
    ...
    output_file = sys.argv[2] if len(sys.argv) > 2 else atomic_file.replace('.json', '_analyzed.json')
